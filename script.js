...
const qFaktur = query(collection(db, 'invoices'), orderBy('tgl', 'desc'));
    onSnapshot(qFaktur, (snapshot) => {
        ...
        let totalNilai = 0; // Ini akan menampung total dari Kurang Pajak
        ...
        invoicesArray.forEach((inv) => {
            ...
            // Hitung Kurang Pajak terlebih dahulu untuk setiap baris
            const nilaiFaktur = Number(inv.nilai);
            const kurangPajak = (nilaiFaktur / 1.11) - ((nilaiFaktur / 1.11)*0.015);

            // Jika status Sudah Bayar, tambahkan nilai "kurangPajak" ke totalNilai
            if (isLunas) {
        ...
        // Menampilkan total rupiah berdasarkan jumlah Kurang Pajak
...
if (document.getElementById('tableBarangBody')) {
    ...
    // Fungsi untuk menambah baris input barang di modal
    ...
    window.openBarangModal = function() {
        ...
        addItemRow(); // Munculkan 1 baris input default saat modal dibuka
    }
    ...
    onSnapshot(qBarang, (snapshot) => {
        ...
        barangArray.forEach((item) => {
            // Jika data lama menggunakan format array (items)
            if (item.items && Array.isArray(item.items)) {
                item.items.forEach((subItem, index) => {
                    const row = document.createElement('tr');
                    row.className = "text-xs sm:text-sm";
                    
                    // Kolom No. Surat Jalan, Tanggal, dan Konsumen hanya ditampilkan di baris pertama untuk surat jalan tersebut
    ...
    // Submit form simpan data barang
    document.getElementById('barangForm').addEventListener('submit', async (e) => {
        ...
        // Ambil semua item barang dari baris-baris input dinamis
